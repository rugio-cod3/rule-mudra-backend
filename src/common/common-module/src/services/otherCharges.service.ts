...
class OtherChargesService {
  ...
  public async findOne(
    ...
    try {
      let otherCharges = await this.otherChargesModel.getOtherCharges(where, order, select)
  ...
  public async countRows(where: {}): Promise<number | ICustomResponse> {
    try {
      let otherChargescount = await this.otherChargesModel.countOtherCharges(where)
  ...
  public async createv2(
    ...
    let insertId = await this.otherChargesModel.insertv2(
      emiID,
      creditID,
      amount,
      customerID,
