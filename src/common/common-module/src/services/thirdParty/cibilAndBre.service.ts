...
import {
  IBureauApiResponse,
  IBureauServiceResponse,
  IDtreeBreResponse,
  IOutputStatus,
} from '../../interfaces/bureauData.interface'
import { IAccount, ICreditReportData, IExperianReportData } from '../../interfaces/cibil.interface'
import { IAccountFormateData, IPaymentHistory } from '../../interfaces/php.interface'
...
const BUREAU_V2_SUPPLIER_ID = 11
const LONG_TERM_DEFAULT_ROI = config.bureauLongTermDefaultRoi
const LONG_TERM_OFFER_ROI = config.bureauLongTermOfferRoi
const LONG_TERM_SWITCH = Number(config.bureauLongTermSwitch)
...
const leadApiLogMongoDBService = new LeadApiLogMongoDBService()
const customerService = new CustomerService()
const callHistoryLogModel = new CallHistoryLogModel()
const callHistoryLogService = new CallHistoryLogService()

export async function breCibilExperianFlow(
  leadID: number,
  customerID: number,
): Promise<{ status: string; message: string }> {
  try {
    let output = await cibilExperianBureau(leadID)

    if (config.nodeEnv !== 'PROD') {
      const apiName = 'Rajat_v1'
      const type = 'bureau'
      output = await finboxService.apiByPassForTesting(apiName, type)
    }

    if (output.Decision === 'Approve' && output.offerAmount > 999) {
      const remarkApprove = 'Approved from BRE New Flow.'
      await finboxService.newCustomerAutoApproveFinbox(leadID, output.offerAmount, remarkApprove)
    ...
    if (output.Decision === 'Reject') {
      const rejectremark = 'Reject Process from BRE New Flow.'
      await approvalService.rejectProcessCustomer(leadID, rejectremark)
      return { status: '1', message: 'Reject' }
    }

    if (output.Decision === 'Not Eligible') {
      const rejectremark = 'Not Eligible from BRE New Flow.'

      await leadService.updateOne({ leadID }, { status: LeadStatus.NOT_ELIGIBLE })

      await callHistoryLogModel.insert({
        customerID,
        leadID,
        callType: 'IVR',
        status: 'Not Eligible',
    ...
    return { status: '0', message: 'Error in processing' }
  }
}
...
export async function autoApproveNewCustomerUsingCibilAndBRE(
  ...
  try {
    ...
    const customerStatus = await customerService.checkCustomerStatusPhpApi(idNumber)

    if (customerStatus == 0) {
      const rejectremark = 'Active loan on Partner Portal.'
      await approvalService.rejectProcessCustomer(leadID, rejectremark)
      return { status: '1', message: 'Reject' }
    } else if (customerStatus == 1) {
      const response = await breCibilExperianFlow(leadID, customerID)
      return response
    }

    address = await trimAddressCibil(address)
    ...
    if (
      (responseData.errorcode === '1' && responseData.errorMsg === 'Successfully') ||
      (responseData.status === '1' && responseData.message === 'Successfully Cibil Fetch')
    ) {
      let score = 0
      ...
      return await breNewFlowRajatApi(
        leadID,
        mobile,
        idNumber,
        pincode,
...
export async function trimAddressCibil(fullAddress: string): Promise<string> {
...
export async function rajatOldApiWithBre(
  ...
  try {
    ...
    if (score >= 601) {
      try {
        ...
        if (responseArr.status == '1' && responseArr.amount_offered > 999) {
          ...
          await finboxService.newCustomerAutoApproveFinbox(leadID, amountOffered, remarkApprove)
    ...
    logger.error('Error in rajatOldApiWithBre:', error)
  }
}

export async function breNewFlowRajatApi(
  leadID: number,
  mobile: string,
  idNumber: string,
  pincode: string,
  ...
  try {
    if (score >= parseInt(config.cibilScoreBreCheck)) {
      let output = await bureauDataservice.bureauV1(leadID)

      // Simulate testing mode bypass if not in PROD
      if (config.nodeEnv !== 'PROD') {
        const apiName = 'Rajat_v1'
        const type = 'bureau'
        output = await finboxService.apiByPassForTesting(apiName, type)
      ...
      if (output?.Decision === 'Approve' && output?.offerAmount && output.offerAmount > 999) {
        const remarkApprove = 'Approved from New BRE V1.'
        const amountOffered = output.offerAmount
        ...
        await callHistoryLogService.insertCallHistoryLog(leadID, rejectRemark)

        const fallbackOutput = await rajatOldApiWithBre(
          leadID,
          mobile,
          idNumber,
          pincode,
        ...
        if (fallbackOutput.status === '1') {
          return fallbackOutput
        } else {
          const finalRejectRemark = 'BRE and DtreeBre Reject Case Move to Reject Process.'

          await approvalService.rejectProcessCustomer(leadID, finalRejectRemark)
...
async function getDtreeBre(
  ...
  try {
      ...
      if (
        ...
        if (
          ...
          if (amount_offered > 999) {
            ...
            await finboxService.newCustomerAutoApproveFinbox(leadID, amount_offered, remarkApprove)
            status = '1'
            message = 'Success'
          } else {
            status = '0'
    ...
    try {
      await leadApiLogMongoDBService.create('cibil', saveObject)
    } catch (error) {
...
async function getCreditReportData(reportId: number): Promise<ICreditReportData[]> {
  let db = getKnexInstance()
  await db.raw("SET sql_mode=(SELECT REPLACE(@@sql_mode,'ONLY_FULL_GROUP_BY',''))")
...
const getStatusFromPercentage = (percentage: number) => {
  ...
  const status = statusRanges.find(range => percentage >= range.min && percentage <= range.max)
...
const getCreditUsageStatus = (utilizationPercent: number): string => {
  ...
  const status = usageRanges.find(
    range => utilizationPercent >= range.min && utilizationPercent <= range.max,
  )
...
export async function makeCreditReport(
  ...
  let usedLimit = Math.round(
    (parseInt(reportData[0].tot_credit_limit) * parseInt(reportData[0].utilization_percent)) / 100,
  )
  ...
  const creditStatus = getCreditUsageStatus(Math.round(Number(reportData[0].utilization_percent)))
  const ageStatus = getCreditAgeStatus(age)
  const accountStatus = getAccountStatus(activeAccounts)
  const enquiryStatus = getInquiryStatus(parseInt(reportData[0].tot_credit_limit))
...
export async function getAccountData(reportId: number): Promise<Record<any, any>> {
...
const getBankLogo = async (bankName: string): Promise<string | null> => {
  let db = getKnexInstance()
  const result = await db.raw('SELECT pngIcon FROM bankList WHERE LOWER(bankName) = LOWER(?)', [
    bankName?.trim(),
  ])
...
export async function makeAccountDetails(reportId: number): Promise<Record<string, any>> {
  ...
  const transformedData = await Promise.all(
    accountData.map(async (account: IAccount, index: number) => {
      return {
        id: (index + 1).toString(),
        icon: (await getBankLogo(account.bank_name)) ?? config.defaultBankLogoExperian,
        name: account.bank_name,
        remark: 'Timely Payment',
        issueOn: account.opening_date.toISOString().split('T')[0],
        payment: `${account.on_time_payments}/${
  ...
  const usedPercentages = accountData.map((account: IAccount) => {
    if (account.credit_limit > 0) {
      return (
        ((account.credit_limit - account.current_balance) / account.credit_limit) *
        100
      ).toFixed(0)
    } else {
      return '0'
...
export async function getRepaymentData(reportId: number): Promise<IPaymentHistory[]> {
  let paymentHistory = await crProfileRepaymentModel.findAll(
    { report_id: reportId },
    ['profile_account_id', 'repayment_status', 'repayment_date', 'account_type', 'customerID'],
    [{ column: 'id', order: 'desc' }],
  )
...
export async function getDataForRepayment(reportId: number): Promise<IAccountFormateData[]> {
  let accountDetails = await crProfileAccountModel.findAll(
    { report_id: reportId },
    ['id', 'loan_amount', 'credit_limit', 'current_balance', 'account_type', 'account_status'],
    [{ column: 'id', order: 'desc' }],
  )
  return accountDetails
}
  ...
  dateFormateData.forEach((data, index) => {
    ...
    paymentHistoryForAccount.forEach(payment => {
      ...
      paymentHistoryByYear[year][month] =
        payment.repayment_status === '000' || payment.repayment_status === 'XXX' ? true : false
    })

    const usedLimit = data.credit_limit ? data.credit_limit - data.current_balance : 0
    ...
    result[(index + 1).toString()] = {
      details: {
        data: [
          { key: 'Used Percentage', value: `${usedPercentage}%` },
          {
            key: 'Limit Used',
            value: `₹${usedLimit?.toLocaleString()}`,
          },
          {
            key: 'Credit Limit',
            value: data.credit_limit ? `₹${data?.credit_limit?.toLocaleString()}` : '0',
          },
        ],
...
export async function cibilExperianBureau(leadID: number): Promise<IBureauServiceResponse> {
  const output: IBureauServiceResponse = {
    Decision: '',
    offerAmount: 0,
  }

  try {
    const customerData = await customerService.findCustomerInputData(leadID)
    ...
    const customerToken = await customerService.insertAndUpdateCustomerToken(
      customerData.customerID,
    )

    const reference_id = `${customerData.customerID}|${leadID}`
    ...
    const apiResponse = response.data as IBureauApiResponse
    ...
    if (!apiResponse.error && apiResponse.output_data?.rules_output?.final_decision?.Decision) {
      const finalDecision = apiResponse.output_data.rules_output.final_decision

      // Handle long term loan logic
      const countLongTermLoan = await bureauDataservice.countLongTermEligibleLoans()
      const new_roi =
        countLongTermLoan % LONG_TERM_SWITCH === 0 ? LONG_TERM_OFFER_ROI : LONG_TERM_DEFAULT_ROI

      // Save bureau data
      const bureauSaveData = {
        customerID: customerData.customerID,
        leadID,
        reference_id,
        Decision: finalDecision?.Decision ?? null,
        LoanAmount: finalDecision?.LoanAmount ?? null,
        version: 'v1',
        emi_eligible: finalDecision?.emi_eligible ?? null,
        emi_max_tenure: finalDecision?.emi_max_tenure ?? null,
        emi_max_monthly_amt: finalDecision?.emi_max_monthly_amt ?? null,
        api_version: finalDecision?.version ?? null,
        long_term_emi_eligible: finalDecision?.long_term_emi_eligible ?? null,
        long_term_tenure: finalDecision?.long_term_tenure ?? null,
        long_term_loan_amount: finalDecision?.long_term_loan_amount ?? null,
        long_term_loan_roi: finalDecision?.long_term_loan_roi ?? null,
        approval_roi: finalDecision?.long_term_loan_roi ? new_roi : null,
      }
      ...
      const analyticsData = {
        ...
        user_id: apiResponse.user_id ?? null,
        reference_id: apiResponse.reference_id ?? null,
        workflow_version_path: apiResponse.workflow_version_path ?? null,
        engine_history: apiResponse.engine_history
          ? JSON.stringify(apiResponse.engine_history)
          : null,
        output_features: apiResponse.output_data?.features?.output_features
          ? JSON.stringify(apiResponse.output_data.features.output_features)
          : null,
        rules_output: apiResponse.output_data?.rules_output
          ? JSON.stringify(apiResponse.output_data.rules_output)
          : null,
        Decision: finalDecision?.Decision ?? null,
        LoanAmount: finalDecision?.LoanAmount ?? null,
        rule_engine_name: finalDecision?.rule_engine_name ?? null,
        rule_engine_version: finalDecision?.version ?? null,
        DecisionReason: finalDecision?.DecisionReason ?? null,
      }

      await bureauDataservice.create(bureauSaveData)
    ...
    // Save API log
    await leadApiLogService.create(saveData)

    await leadApiLogMongoDBService.create('internalApi', saveData)
  ...
  async function saveAnalyticsData(saveData) {
    try {
      const knex = getKnexInstance()
      const result = await knex('bureau_banking_analytics_data').insert(saveData)
