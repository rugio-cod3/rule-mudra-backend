...
class BureauDataService {
    ...
    try {
      ...
      if (cibilData) {
        ...
        if (cibil && cibil.apimsg) {
          ...
          const reference_id = `${
            cibilData.customerID
          }|${leadID}|${Date.now()}${Math.floor(Math.random() * 9000) + 1000}`
  ...
  public async countLongTermEligibleLoans(): Promise<number> {
    return await this.bureauDataModel.countLongTermEligibleLoans()
  }
}
