...
export default class RazorpayPayoutAccountsModel {
  ...
  async findOne(
    params: KnexFindParams<IRazorPayPayoutAccountsModel, TSelectRazorPayPayoutAccountsModel>,
  ...
  async insert(data: InsertData<IRazorPayPayoutAccountsModel>): Promise<number[]> {
    let db = getKnexInstance()
