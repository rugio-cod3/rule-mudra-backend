...
class EMIHelper {
  async emiGenerator(
    ...
    if (dueDate && disbursalDate) {
      bpiCharges = await this.bpiCalculator(
        Principal,
        ROI,
        dueDate,
        disbursalDate,
    ...
    monthlyBpiCharges = Math.round(bpiCharges / Tenure)
    ...
    for (let month = 1; month <= totalMonths; month++) {
      let monthlyInterest =
        Math.round(remainingPrincipal * monthlyInterestRate) + monthlyBpiCharges
  ...
  public bpiCalculator = async (
    ...
    const disbursalDate = disbursalDateString
      ? new Date(disbursalDateString)
      : currentDate
  ...
  public amountCalculatorForPreClosure = async (
    creditID: number,
  ): Promise<number> => {
