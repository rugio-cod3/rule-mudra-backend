...
import { apiByPassModel } from "@/database/mysql/api_bypass";
...
import { DateDifference, nameCheckPercentage } from "@/enums/finbox.enum";
...
import { repayDateholidayservice } from "../repayDateHoliday.service";
...
class FinboxService {
  ...
  private readonly customerNameMatchService = customerNameMatchservice;
  ...
  private readonly repayDateholidayService = repayDateholidayservice;
  ...
  async predictors(
    ...
    try {
      let check = await this.leadApiLogService.findOne(
        {
          status: 1,
          api_supplier: 2,
      ...
      //   is_success: true,
      //   apimsg: {
      //     entity_id: 'd27d1ba2-ba4e-4f39-806c-d61bd09cc022',
      //     progress: [ [Object] ],
      //     predictors: [ [Object] ]
      //   }
      // }
      //failure response
      // { is_success: false, apimsg: { detail: 'Not found.' } }
      if (
        ...
        await this.leadApiLogService.create(saveArray);
  ...
  async bankConnectXlsxReport(entityId: string, customerID: number) {
    ...
    await this.leadApiLogService.create(saveData);
  ...
  async checkNamePercentage(
    ...
    if (shouldSave) await this.customerNameMatchService.create(saveData);
  ...
  async leadStatusChangedDocumentReceivedNew(leadID: number) {
    ...
    const leaddetails = await leadsService.findOne({ leadID });
    ...
    if (
      ...
      await Promise.all([
        this.callHistoryService.create({
          customerID,
          leadID,
      ...
      const leadDataForApprovedProcess = await leadsService.findOne(
        { leadID },
        ["*"],
  ...
  async bankingChecksForFinbox(leadID: number, customerID: number) {
    ...
    const baseUrl = commonHelper.getBaseUrl();
    const apiCall = new AxiosService(baseUrl);

    const mobileTokenService = new MobileTokenService();

    const mobileToken = await mobileTokenService.findOne(
  ...
  async repayDateFind(salaryDate: string) {
    ...
    let holidayCount = await this.repayDateholidayService.findAndCount({
    ...
    while (holidayCount > 0) {
      ...
      holidayCount = await this.repayDateholidayService.findAndCount({
        repaydate: targetDate.format("YYYY-MM-DD"),
      });
