...
import {
    ...
    ICustomerLeadDetails,
    IState,
    ApiBypassTypes
} from '../interfaces/experian.interface'

class NewExperianService extends ResponseService {
    private stateModel = new StateModel()
    private leadsApiLogModel = leadsApiLogModel
    ...
    private async getExperianDetails(
        ...
        // Always make the real API call (bypass logic removed)
    ...
    private convertToExperianResponse(data: IExperianResponse): IServiceResponse {
        const userMessage = data.INProfileResponse?.UserMessage?.UserMessageText

        // Handle valid responses: Normal Response or No record found
        if (userMessage === 'Normal Response' || userMessage?.includes('No record found')) {
            return {
                statusCode: HttpStatusCode.Ok,
                data,
                message: userMessage?.includes('No record found') ? 'No credit history found' : 'Success',
            }
        }
        ...
        return {
            ...
            data: {},
            message: `Unable to fetch experian data: ${userMessage || 'Unknown error'}`,
        }
    }

    private convertToBureauExperianResponse(
        details: IExperianResponse,
    ): IExperianResponse | IServiceResponse | null {
