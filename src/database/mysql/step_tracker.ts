...
import { stepControlModel } from './step-control'
...
export default class StepTrackerModel {
  ...
  async createStepForLoanOnboarding(
    payload: IUpdateStep,
  ): Promise<ICreateStepTrackerForProduct> {
    const { customerID, stepId, currentStepOrder, productId, isCompleted } =
      payload
  ...
  async completeStep(
    ...
    if (!productDetails) {
      logger.error(
        'Error saving step in step_tracker \n Reason: Product details not found',
      )
      return
    }
    ...
    if (
      step.provider_id === StepProvider.REPEAT_CUSTOMER &&
      step.step_order === 1
    ) {
    ...
    const lead = leadID
      ? await this.leadModel.findOne({ where: { leadID } })
      : null
    ...
    if (
      ...
      leadType === LeadType.EXISTING_CASE
    ) {
  ...
  async saveFinboxIncompleteStep(
    customerID: number,
    leadID: number,
    product: Products,
  ) {
