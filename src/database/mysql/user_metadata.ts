...
import {
  ...
  TSelectUserMetaData,
} from '@/interfaces/user_metadata.interface'
import {
  InsertData,
  SelectFields,
  SortCriteria,
...
export default class UserMetaDataModel {
  ...
  public async getUserMetaDatas(
    ...
    try {
      ...
      logger.error(
        'Error Inside user_metadata.ts getUserMetaDatas function',
        error,
      )
    }
  }
  ...
  public async getUserMetaData(
    ...
    try {
      ...
      logger.error(
        'Error Inside user_metadata.ts getUserMetaData function',
        error,
      )
    }
  }
    ...
    if (userMeta) {
      ...
      switch (type) {
        case LeadLogApiType.PAN_COMPREHENSIVE:
          ...
          if (
            isUserMetaExists &&
            !userMetaJson[LeadLogApiType.PAN_COMPREHENSIVE]
          ) {
        ...
        case LeadLogApiType.AADHAR_V2_SUBMIT_OTP:
          ...
          const userMetaSurepassAadharJson: IUserMetaSurePassAadhar = {
            [LeadLogApiType.AADHAR_V2_SUBMIT_OTP]: {
              ...
              }/${surePassAadharAddress.dist ?? ''}/${
                surePassAadharAddress.state ?? ''
              }/${surePassAadharAddress.po ?? ''}/${
                surePassAadharAddress.loc ?? ''
              }/${surePassAadharAddress.vtc ?? ''}/${
                surePassAadharAddress.subdist ?? ''
              }/${surePassAadharAddress.street ?? ''}/${
                surePassAadharAddress.house ?? ''
          ...
          if (
            isUserMetaExists &&
            !userMetaJson[LeadLogApiType.AADHAR_V2_SUBMIT_OTP]
          ) {
        ...
        case LeadLogApiType.DIGILOCKER_EAADHAR:
          ...
          const userMetaDigiAadharJson: IUserMetaDigilockerAadhar = {
            [LeadLogApiType.DIGILOCKER_EAADHAR]: {
              ...
              address: `${country ?? ''}/${district ?? ''}/${state ?? ''}/${
                postOffice ?? ''
              }/${locality ?? ''}/${vtc ?? ''}/${subDistrict ?? ''}/${
                street ?? ''
              }/${house ?? ''}/${landmark ?? ''}`,
            },
          }
          ...
          if (
            isUserMetaExists &&
            !userMetaJson[LeadLogApiType.DIGILOCKER_EAADHAR]
          ) {
      ...
      switch (type) {
        ...
        case LeadLogApiType.AADHAR_V2_SUBMIT_OTP:
          ...
          await this.insert({
            ...
            aadhar_mask:
              userMetaSurepassAadharJson['aadhaar-v2-submit-otp'].maskAadhar,
          })
