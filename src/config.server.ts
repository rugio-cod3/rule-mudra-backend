...
// PreProd env
// require("dotenv").config({ path: cwd() + "/preprod.env" });
...
import { flattenObject, unFlattenObject, mergeDeep } from "@/helpers/object";
...
function assignEnvs(customEvnVariables: object, nodeEnvObject: Object) {
  ...
  Object.keys(flattedObject).forEach((variable) => {
    if (nodeEnvObject[flattedObject[variable]] !== undefined) {
      ...
      throw new Error(
        `Env variable ${flattedObject[variable]}, is not defined`
      );
    }
  });
...
const configInitialized = () => {
  ...
  const environmentConstants: DeepPartial<IConfigApp> =
    getEnvironmentConstants(nodeEnv);
  constants = mergeDeep(constants, environmentConstants, environmentVariables) as IConfigApp;
  return constants;
};
